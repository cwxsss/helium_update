...
func chromeUpdaterNew(sd *SettingsData, updateUrl binding.String, newBtn *widget.Button) error {
	...
	if len(githubReleases) != 0 {
		...
		for _, asset := range githubReleases[0].Assets {
			if strings.Contains(asset.BrowserDownloadURL, fmt.Sprintf("helium_updater-windows-%s.zip", runtime.GOARCH)) {
				logger.Debugf("Helium updater url:%s", asset.BrowserDownloadURL)
