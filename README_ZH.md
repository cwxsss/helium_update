...
- 将官方包解压并整理为便携目录，目标目录指向包含 `chrome.exe` 的 `Application` 文件夹。
- 可单独检查和下载 Helium++；仅从发布包解压 `version.dll` 并保存为 `chrome.exe` 同级的 `version.dll.new`，不会修改 `chrome++.ini`。
...
4. 主程序和 Helium++ 可以分别更新，互不影响。Helium++ 下载完成后，关闭 Helium 和本更新器，再在 `Application` 目录中手动用 `version.dll.new` 覆盖 `version.dll`。
